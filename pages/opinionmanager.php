...
if (isset($_POST['id']) && isLoggedIn()) {
    ...
    $opinion = ($type == 0) ? 2 : 1;
    $postcontent = mysqli_fetch_array(query("SELECT SUBSTRING(Posts.content, 1, 30) FROM Posts WHERE Posts.id = '$post' LIMIT 1"))[0];
    if (isset($_POST['add'])) {
        ...
        if ($opinion == 1) newNotification($friend, $user, $post, $opinion, $postcontent."...");
    } else {
        $sql = query("DELETE FROM Opinions WHERE result = (1000000000*'$user')+'$post'");
        if ($opinion == 1) deleteNotification($friend, $user, $post, $opinion, $postcontent."...");
    }
    if ($sql) {
?>
{"CODE": 200}
