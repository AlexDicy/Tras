(function (wHandle, wjQuery) {
    ...
    function gameLoop() {
        ...
        wHandle.onblur = function () {
            sendUint8(19);
            sPressed = fPressed = wPressed = qPressed = spacePressed = false
        };
    ...
    function onTouchStart(e) {
        ...
        for(var i = 0; i<e.changedTouches.length; i++){
            ...
            if((leftTouchID<0) && (touch.clientX<canvasWidth))
            {
    ...
    function getServerList() {
        ...
        wjQuery.get("//tras.pw/info.php", function (a) {
            var numPlayers = {};
    ...
    function attemptConnection() {
        wjQuery.ajax("//sockets.tras.pw/3839/main", {
            error: function () {
                setTimeout(attemptConnection, 1E3)
            },
            success: function (a) {
                CONNECTION_URL = a.split("\n")[0]; 
                if (CONNECTION_URL != "0.0.0.0") {
                    wsConnect(localProtocolHttps ? "wss://" + CONNECTION_URL : "ws://" + CONNECTION_URL);
                    //console.log(localProtocolHttps ? "wss://" : "ws://");
                } else {
                    console.log("region not found - error");
                }
                // + gameMode || "?"
            },
            dataType: "text",
    ...
    wHandle.populateRgs = populateRegions();
    function populateRegions() {
        wjQuery.ajax("//sockets.tras.pw/3839/info", {
            error: function () {
                setTimeout(populateRegions(), 1E3)
            },
            success: function (a) {
                var opts = wjQuery.parseJSON(a);
                $.each(opts, function(i, d) {
                    $('#region').append('<option value="' + d.regions + '">' + d.regions + d.regions.numPlayers + '</option>');
                });
            },
            dataType: "json",
            method: "GET",
    ...
    function wsConnect(wsUrl) {
        ...
        //wsUrl = localProtocolHttps ? "wss://" : "ws://" + c;
        nodesOnScreen = [];
        playerCells = [];
        nodes = {};
        nodelist = [];
    ...
    function drawGameScene() {
        ...
        if (0 != userScore && showScore) {
            if (null == scoreText) {
                scoreText = new UText(18, '#FFFFFF');
            }
            scoreText.setValue('Highest: ' + ~~(userScore / 100) + ' Mass: ' + ~~(calcUserScore() / 100));
            c = scoreText.render();
            a = c.width;
            ctx.globalAlpha = .2;
    ...
    function drawGrid() {
        //#F2FBFF #111111 #000000 #AAAAAA
        ctx.fillStyle = showBlueTheme ? "#000963" : "#111111";
        ctx.fillRect(0, 0, canvasWidth, canvasHeight);
        ctx.save();
        ctx.strokeStyle = showBlueTheme ? "#000000" : "#AAAAAA";
        ctx.globalAlpha = .2;
        ctx.scale(viewZoom, viewZoom);
    ...
    function drawLeaderBoard() {
        ...
        if (null != teamScores || 0 != leaderBoard.length)
            if (null != teamScores || showName) {
                ...
                var c = "Scoreboard";
                ctx.font = "30px Lalezar, Verdana, Geneva, sans-serif";
                ctx.fillText(c, 100 - ctx.measureText(c).width / 2, 40);
                var b;
                if (null == teamScores) {
                    for (ctx.font = "20px Lalezar, Verdana, Geneva, sans-serif", b = 0; b < leaderBoard.length; ++b) {
                        c = leaderBoard[b].name || "404 Not found";
                        if (!showName) {
                            (c = "404 Not found");
                        }
    ...
    function bootBackground(){
        var id = 666;
        //#ff2907
        var node = new Cell(id, nodeX - 30, nodeY - 45, 600, "#ffdf07", "Tras");
        nodelist.push(node);
        nodes[id] = node;
        node.ka = nodeX - 30;
        node.la = nodeY - 45;
        node.isVirus = false;
        node.isAgitated = true;
        node.nx = 3000;
        node.ny = 3000;
        node.nSize = 600;
        node.updateCode = 0.22267717147156652;
        node.updateTime = 1476471322420;
        node.flag = 0;
        playerCells.push(node);
        //nodeX = (29 * nodeX + node.x) / 30;
        //nodeY = (29 * nodeY + node.y) / 30;
        //nodeX = node.x;
        //nodeY = node.y;
        //node.setName("Tras");
    }
    ...
    var nCanvas, ctx, mainCanvas, lbCanvas, chatCanvas, canvasWidth, canvasHeight, qTree = null,
        ...
        nodelist = [],
        Cells = [],
        leaderBoard = [{"id": 1,"name": "Tras.pw"}, {"id": 2,"name": "Welcome, press 'Play'"}, {"id": 3,"name": "to start a new game"}],
        chatBoard = [],
        rawMouseX = 0,
        rawMouseY = 0,
        X = -1,
        ...
        noRanking = true;
        showScore = false;
    splitIcon.src = "//tras.pw/assets/images/split.png";
    ejectIcon.src = "//tras.pw/assets/images/fire.png";
    var wCanvas = document.createElement("canvas");
    var playerStat = null;
    wHandle.isSpectating = false;
    wHandle.disconnect = function () {
        if (ws) {
            ws.onopen = null;
            ws.onmessage = null;
            ws.onclose = null;
    ...
    wHandle.toggleDisconnect = function (arg) {
        if (arg) {
            wjQuery("#disconnectBtn").fadeOut("fast", function() {
                wjQuery("#playBtn").css("width", "85.7%");
                wjQuery("#playBtn").css("margin-right", "4px");
            });
        } else {
            wjQuery("#playBtn").css("width", "51.5%");
            wjQuery("#playBtn").css("margin-right", "0");
            wjQuery("#disconnectBtn").fadeIn().css("display","inline-block");
        }
    };
    ...
    wHandle.setBlueTheme = function (arg) {
        showBlueTheme = arg
    };
    wHandle.setGrayScale = function (arg) {
        grayScale = arg
    };
    wHandle.setShowMass = function (arg) {
        showMass = arg
    };
    wHandle.setLessLag = function (arg) {
        lessLag = arg ? 2 : .4
    };
    ...
    var data = {"action": "getSkins"};
    //var response = null;
    wjQuery.ajax({
        type: "POST",
        dataType: "json",
        url: "//tras.pw/skins.php", //Relative or absolute path to response.php file
        data: data,
        success: function (data) {
            //alert(data["names"]);
            //response = data["names"];
            knownNameDict = data['nicks'];
        }
    });
    ...
    Cell.prototype = {
        ...
        ny: 0,
        nSize: 0,
        flag: 0, //what does this mean, lol
        updateTime: 0,
        updateCode: 0,
        drawTime: 0,
        destroyed: false,
        ...
        movePoints: function () {
            ...
            for (var ref = this, isvirus = this.isVirus ? 0 : (this.id / 1E3 + timestamp / 1E4) % (2 * Math.PI), j = 0; j < numpoints; ++j) {
                ...
                //if (this.isVirus) e = 0.063;
                //this.color = "#ff2300";
                m = this.points[j].size;
                this.isVirus && 0 == j % 4 && (m += 50);
        ...
        drawOneCell: function (ctx) {
            if (this.shouldRender()) {
                var b = (0 != this.id && !this.isVirus && !this.isAgitated && lessLag /*> 1 */> viewZoom * 1.7);
                ...
                if (10 == this.size){
                    aaaa = true;
                    b = false;
                } else if (20 == this.size){
                    isBullet = true;
                    b = true;
                }
                //if (10 < this.size < 100) console.log(this.size);
                ...
                ctx.lineWidth = 10;
                ctx.lineCap = "round";
                ctx.lineJoin = this.isVirus ? "bevel" : "round";
                if (grayScale) {
                    ctx.fillStyle = "#FFFFFF";
                ...
                if (b) {
                    ...
                    //if (isBullet);
                    var point0x, point0y;
                    if(typeof this.points !== 'undefined' || typeof this.points !== 'undefined') {
                            if(typeof this.points[0] !== 'undefined' || typeof this.points[0] !== 'undefined') {
                                if(typeof this.points[0].x !== 'undefined' || typeof this.points[0].y !== 'undefined') {
                                    if (this.points[0].x == null || this.points[0].y == null) {
                                        console.log("NULL!!!");
                                        point0x = 0;
                                        point0y = 0;
                                    } else {
                                        point0x = this.points[0].x;
                                        point0y = this.points[0].y;
                                    }
                                } else {
                                    point0x = this.points[0].x;
                                    point0y = this.points[0].y;
                    ...
                    for (c = 1; c <= d; ++c) {
                        var e = c % d;
                        if(typeof this.points !== 'undefined' || typeof this.points !== 'undefined') {
                            if(typeof this.points[e] !== 'undefined' || typeof this.points[e] !== 'undefined') {
                                if(typeof this.points[e].x !== 'undefined' || typeof this.points[e].y !== 'undefined') {
                                    if (this.points[e].x == null || this.points[e].y == null) {
                                        console.log("NULL!!!");
                                        alert("I'm sorry but you may be crashed, Don't report this bug as it's known, thank you.");
                                        pointx = 0;
                                        pointy = 0;
                                    } else {
                                        pointx = this.points[e].x;
                                        pointy = this.points[e].y;
                                    }
                                } else {
                                    pointx = this.points[e].x;
                                    pointy = this.points[e].y;
        ...
        render: function () {
            ...
            if (this._dirty) {
                ...
                var canvas = this._canvas,
                    ...
                    scale = this._scale,
                    fontsize = this._size,
                    font = fontsize + 'px Lalezar, Verdana, Geneva, sans-serif';
                ctx.font = font;
                var h = ~~(.2 * fontsize);
    ...
    wjQuery(function () {
        ...
        var redCell = new Cell(0, 0, 0, 128, "#ED1C24", ""),
...
function showImg() {
    var img = $("<img id=\"cannotDisplayAd\">");
    img.attr("src", "//tras.pw/assets/images/blocker.png");
    img.appendTo("#leftAd");
    $("#leftAd").height("100px");
}
...
function panelResize() {
    ...
    if (wWidth >= pWidth && wHeight >= pHeight) {
        panel.css({'-webkit-transform': 'translate(-50%, -50%)'});
        panel.css({'transform': 'translate(-50%, -50%)'});
        panel.css({'-ms-transform': 'translate(-50%, -50%)'});
    } else {
        acc666 = Math.min(
            wWidth / panel.width(), 
            wHeight / panel.height() 
        );
        panel.css({transform: "translate(-50%, -50%) " + "scale(" + acc666 + ")"});
    }
}

function nickChange(val) {
    var id = $(val).data("box-id");
    Cookies.set("checkbox-" + id, $(val).val(), {
        path: '/',
        expires: 365
    });
}
...
$(window).load(function() {
    ...
    $(".save").each(function() {
        ...
        var cookie = Cookies.get("checkbox-" + id);
        if (cookie && cookie == "true" && 0 != id) {
            $(this).prop('checked', cookie);
            $(this).trigger("change");
        } else if (id == 0 && cookie != null) {
            $(this).val(cookie);
        }
    });
    $(".save").change(function() {
        var id = $(this).data('box-id');
        if (id != 0) {
            Cookies.set("checkbox-" + id, $(this).prop('checked'), {
                path: '/',
                expires: 365
            });
        }
